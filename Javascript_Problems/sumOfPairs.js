...
// sum_pairs([0, 0, -2, 3], 2) == None / nil / undefined;
...
const sumPairs = (integers, sum) => {
  const sumToReturn = [];
  const arrLength = integers.length;
  for (let i = 0; i < arrLength; i++) {
    for (let z = i; z < arrLength; z++) {
      let indexOfI = integers.indexOf(integers[i]);
      let indexOfZ = integers.indexOf(integers[z]);
      console.log("pairs ====>", integers[i], integers[z]);
      console.log('indexOfI ===>', indexOfI)
      console.log('indexOfZ===>', indexOfZ)
      if (integers[i] + integers[z] === sum && indexOfI != indexOfZ) {
        sumToReturn.push(integers[i]);
        sumToReturn.push(integers[z]);
      }
    }
  }
  console.log("sum to return ===>", sumToReturn);

  if (sumToReturn.length > 1) {
    return sumToReturn;
  } else {
    undefined;
  }
...
console.log("sumPairs ===>", sumPairs(l4, 2));
